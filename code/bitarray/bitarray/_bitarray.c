...
static binode * parse_bitarray_tree(bitarrayobject *bitarray_tree, idx_t quantization_bits, idx_t *index) {
	
	// Returns subtree constructed from this index
	// Based on https://stackoverflow.com/questions/759707/efficient-way-of-storing-huffman-tree
	// In case of error: deletes any part of the subtree that may already have been constructed and returns NULL 
	...
	int case_bit = GETBIT(bitarray_tree, *index);
	(*index)++;
	if (case_bit) {
		...
		for (; *index < max_index; (*index)++) {
			symbol_value <<= 1;
			symbol_value += GETBIT(bitarray_tree, *index);
		}
		binode *subtree = new_binode();
		subtree->symbol = PyLong_FromLong(symbol_value);
		return subtree;
		
	} else {
		
		// Non-leaf
		// First parse leaves in case of error
		
		binode *child0 = parse_bitarray_tree(bitarray_tree, quantization_bits, index);
		if (!child0) {
			// Error occurred, propagate, nothing needs to be freed yet
			return NULL;
		}
		binode *child1 = parse_bitarray_tree(bitarray_tree, quantization_bits, index);
		if (!child1) {
			// Error occurred, propagate, child0 needs to be freed first
			delete_binode_tree(child0);
			return NULL;
		}
		
		// All bound checks have finished now
		binode *subtree = new_binode();
		subtree->child[0] = child0;
		subtree->child[1] = child1;
		return subtree;
...
static PyObject *
bitarray_iterdecodetree(bitarrayobject *self, PyObject *args)
{
	decodeiterobject *it; // iterator to be returned
	binode *tree;
	
	// Parse and check input
	PyObject *bitarray_tree, *quantization_bits;
	if (!PyArg_ParseTuple(args, "OO:encode", &bitarray_tree, &quantization_bits))
		return NULL;
    if (!bitarray_Check(bitarray_tree)) {
        PyErr_SetString(PyExc_TypeError, "bitarray expected as first argument for iterdecodetree");
        return NULL;
    }
	if (!PyLong_Check(quantization_bits))
		return NULL;
...
PyDoc_STRVAR(iterdecodetree_doc,
"iterdecodetree(code) -> iterator\n\
\n\
Given a tree (represented as a bitarray),\n\
decode the content of the bitarray and return an iterator over\n\
the symbols.");
...
/*********************** (Bitarray) Constant-Length Decode Iterator *********************/

typedef struct {
    PyObject_HEAD
    bitarrayobject *bao;        /* bitarray we're searching in */
    PyObject *codelist;     /* list of codes for constant-length code. maps an index (code, big-endian formatted) to a value (symbol) */
    int bits;					/* bits per code */
    idx_t index;                /* current index in bitarray */
} decodeiterconstantobject;

static PyTypeObject DecodeIterConstant_Type;

#define DecodeIterConstant_Check(op)  PyObject_TypeCheck(op, &DecodeIterConstant_Type)

/* create a new initialized bitarray search iterator object */
static PyObject *
bitarray_iterdecodeconstant(bitarrayobject *self, PyObject *codelist)
{
    decodeiterconstantobject *it;  /* iterator to be returned */

    if (!PyList_Check(codelist))
        return NULL;

    it = PyObject_GC_New(decodeiterconstantobject, &DecodeIterConstant_Type);
    if (it == NULL)
        return NULL;

    Py_INCREF(codelist);
    it->codelist = codelist;
	
	// Calculate bits
	Py_ssize_t size = PyList_Size(codelist);
	it->bits = 0;
	while (size >>= 1)
		it->bits++;
...
PyDoc_STRVAR(iterdecodeconstant_doc,
"iterdecodeconstant_doc(code) -> iterator\n\
\n\
Given a constant length code (each symbol is encoded using the\n\
same amount of bits) as a list of bitarrays of length 2**bits,\n\
decode the content of the bitarray and return an iterator over the symbols.\n\
Indices of code list should be big-endian encoded.");

static PyObject *
decodeiterconstant_next(decodeiterconstantobject *it)
{
	
	// Initialize and check
    assert(DecodeIterConstant_Check(it));
	Py_ssize_t code_value = 0;
	idx_t max_it_index = it->index + it->bits;
	if (max_it_index > it->bao->nbits) {
		PyErr_SetString(PyExc_ValueError, "not enough bits left in iterator for another symbol");
		return NULL;
	}
	
	// Calculate symbol and return
	for (; it->index < max_it_index; it->index++) {
		code_value <<= 1;
		code_value += GETBIT(it->bao, it->index);
	}
	
    PyObject *symbol = PyList_GetItem(it->codelist, code_value);
    Py_INCREF(symbol);
	return symbol;
}

static void
decodeiterconstant_dealloc(decodeiterconstantobject *it)
{
    PyObject_GC_UnTrack(it);
    Py_XDECREF(it->bao);
    Py_XDECREF(it->codelist);
    PyObject_GC_Del(it);
}

static int
decodeiterconstant_traverse(decodeiterconstantobject *it, visitproc visit, void *arg)
{
    Py_VISIT(it->bao);
    Py_VISIT(it->codelist);
    return 0;
}
...
static PyTypeObject DecodeIterConstant_Type = {
    ...
    0,                                        /* ob_size */
#endif
    "bitarraydecodeiteratorconstant",         /* tp_name */
    sizeof(decodeiterconstantobject),         /* tp_basicsize */
    0,                                        /* tp_itemsize */
    /* methods */
    (destructor) decodeiterconstant_dealloc,  /* tp_dealloc */
    0,                                        /* tp_print */
    0,                                        /* tp_getattr */
    0,                                        /* tp_setattr */
    0,                                        /* tp_compare */
    ...
    Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC,  /* tp_flags */
    0,                                        /* tp_doc */
    (traverseproc) decodeiterconstant_traverse,/* tp_traverse */
    0,                                        /* tp_clear */
    0,                                        /* tp_richcompare */
    0,                                        /* tp_weaklistoffset */
    PyObject_SelfIter,                        /* tp_iter */
    (iternextfunc) decodeiterconstant_next,   /* tp_iternext */
    0,                                        /* tp_methods */
};
...
bitarray_methods[] = {
    ...
    {"iterdecode",   (PyCFunction) bitarray_iterdecode,  METH_O,
     iterdecode_doc},
    {"iterdecodetree",   (PyCFunction) bitarray_iterdecodetree,  METH_VARARGS,
     iterdecodetree_doc},
    {"iterdecodeconstant",   (PyCFunction) bitarray_iterdecodeconstant,  METH_O,
     iterdecodeconstant_doc},
